    ...
    <header className={`site-header ${scrolled || location !== "/" ? "site-header--solid" : ""}`}>
      <div className="mx-auto flex h-[4.85rem] max-w-[1480px] items-center justify-between px-5 sm:px-8 lg:px-10">
        ...
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="icon" className="header-icon-button" onClick={() => setLocation("/search")} aria-label={t("nav.openSearch")}><Search size={19} /></Button>
          <a href="https://t.me/MWitcherr" target="_blank" rel="noreferrer" className="hidden h-10 items-center gap-2 rounded-full border border-white/10 bg-white/[0.045] px-4 text-[0.72rem] font-bold tracking-[0.06em] text-[#efede8] transition hover:border-white/20 hover:bg-white/10 sm:inline-flex">
            <Sparkles size={15} className="text-[#e33b2f]" /> {t("nav.telegram")}
          </a>
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button variant="ghost" size="icon" className="header-icon-button" aria-label={t("nav.settings")}><Settings2 size={18} /></Button>
            </DialogTrigger>
            <DialogContent className="max-w-md border-white/10 bg-[#171714] p-0 text-[#f4f0e9] shadow-2xl">
              <DialogHeader className={dir === "rtl" ? "border-b border-white/10 px-6 py-5 text-right" : "border-b border-white/10 px-6 py-5 text-left"}>
                <DialogTitle className="font-display text-xl tracking-[-0.045em]">{t("settings.title")}</DialogTitle>
                <DialogDescription className={dir === "rtl" ? "pt-1 text-right text-[#9d9b96]" : "pt-1 text-left text-[#9d9b96]"}>{t("settings.desc")}</DialogDescription>
              </DialogHeader>
              <div className="space-y-5 px-6 py-6" dir={dir}>
                <label className="block space-y-2">
                  <span className="flex items-center gap-2 text-sm font-semibold"><Globe2 size={15} className="text-[#e33b2f]" /> {t("settings.language")}</span>
                  <select value={draftLocale} onChange={(event) => setDraftLocale(event.target.value as Locale)} className="settings-control">
                    {locales.map((option) => (
                ...
                <label className="block space-y-2">
                  <span className="text-sm font-semibold">{t("settings.username")}</span>
                  <input value={username} onChange={(event) => setUsername(event.target.value)} placeholder={t("settings.usernamePlaceholder")} className="settings-control" />
                </label>
                <Button className="h-11 w-full rounded-xl bg-[#e33b2f] font-bold text-white hover:bg-[#f04b3f]" onClick={persistSettings}>{t("settings.save")}</Button>
              </div>
            </DialogContent>
          </Dialog>
